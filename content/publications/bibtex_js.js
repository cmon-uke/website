    ...
    this.displayAuthor = function(string, format) {
        ...
        for (i = 0; i < searchLength; i++) {
            ...
            for (j = 0, ele = author.find("span:not(a)"), len = ele.length; j < len; ++j) {
                ...
                if (name[index] != "") {
                    if (index == Format.FIRST) {
                        ele[j].textContent = name[Format.FIRST_INITIAL];
                    } else if (ele[j].hasAttribute("bibtex-js-rif") && !textBefore) {
            ...
            container.each(
                function() {
                    if (!funcCaller.checkEntry($(this), word)) {
                        ...
                        var year = $(this).parent().find('h4').text();
                        var container_parent = $(this).parent().parent()
                        ...
                        if (!hasVisibleChildren) {
                            ...
                            container_parent.css("display","flex")
                        }
                    } else {
                        $(this).show();
                        var container_parent = $(this).parent().parent()
                        if (container_parent.css("display") == "none") {
                            container_parent.css("display","flex")
                        }
                        var hasVisibleChildren = false;
                        container_parent.children().each(function() {
                            if ($(this).find(".bibtexentry:visible").length > 0) {
                                hasVisibleChildren = true;
                                return false;
                            }
                        });
...
function generateList(object, bibtexField) {
    ...
    if (bibtexField == "author") {
        ...
        for (var key in map) {
            ...
            // Filter authors based on the pre-built list
            if (authorsList.includes(key)) {
    ...
    if (optionStr !== undefined) {
        ...
        properties.forEach(function(property) {
            ...
            if (tup[1].toLowerCase() === "true") {
                options[tup[0]] = true;
            } else if (tup[1].toLowerCase() === "false") {
                options[tup[0]] = false;
            } else {
                options[tup[0]] = tup[1];
            }
    ...
    var sortOrder = options.descending ? -1 : 1;
